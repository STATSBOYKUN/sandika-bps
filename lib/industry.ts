...
function buildDesaSpatialRecords(): DesaSpatialRecord[] {
  ...
  for (const feature of features) {
    ...
    if (!props?.kode_kec || !props?.kode_kd || !props?.kel_desa || !props?.kecamatan) {
      continue;
    }
...
function inferLocationByKecamatanName(kecamatanNama: string) {
  const found = DESA_SPATIAL_RECORDS.find((item) =>
    normalize(item.kecamatanNama) === normalize(kecamatanNama),
  );
...
function buildGoogleMapsSeeds(content: string): IndustrySeedInput[] {
  ...
  rows.forEach((row, index) => {
      ...
      const locationByCoordinate = inferLocationByCoordinate(latitude, longitude);
      ...
      output.push({
        ...
        metadata: {
          ...
          mapsUrl: row.link || `https://maps.google.com/?q=${latitude},${longitude}`,
...
function buildTikTokSeeds(content: string): IndustrySeedInput[] {
  ...
  rows.forEach((row, index) => {
      const authorId = pickFirst(row, ["author_id", "authorId", "creator_id", "creatorId"]);
      ...
      const videoTitle = pickFirst(row, ["video_title", "videoTitle", "title"]) || `Video TikTok ${index + 1}`;
      const videoUrl = pickFirst(row, ["video_url", "videoUrl", "url"]);
      const sourceId = videoId || videoUrl || `${authorId}-${index + 1}`;
      if (!sourceId) return;

      const namaUsaha = authorUsername || authorId || `TikTok Creator ${index + 1}`;
      const location = {
        ...inferLocationByKecamatanName("Karanganyar"),
        isInsideKaranganyar: true,
      };
      ...
      output.push({
        ...
        metadata: {
          ...
          videoTitle,
          videoUrl: videoUrl || "",
          publishedAt: pickFirst(row, ["published_at", "publishedAt", "created_at", "createdAt"]),
          viewCount: Math.trunc(toNumber(pickFirst(row, ["view_count", "views", "viewCount"]))),
          likeCount: Math.trunc(toNumber(pickFirst(row, ["like_count", "likes", "likeCount"]))),
          commentCount: Math.trunc(
            toNumber(pickFirst(row, ["comment_count", "comments", "commentCount"])),
          ),
          shareCount: Math.trunc(toNumber(pickFirst(row, ["share_count", "shares", "shareCount"]))),
          followerCount: Math.trunc(
            toNumber(pickFirst(row, ["follower_count", "followers", "followerCount"])),
          ),
...
export async function readIndustrySeedData(baseDir = process.cwd()) {
  const googlePath = path.join(baseDir, "public", "data", "Google Maps", "Desain Grafis.csv");
  const youtubePath = path.join(baseDir, "public", "data", "YouTube", "YouTube Data.csv");
  const tiktokPath = path.join(baseDir, "public", "data", "TikTok", "TikTok Data.csv");
...
function mapWilayahFromMetadataOrBase(
  ...
  const provinsiRaw = isObject(wilayahRaw.provinsi) ? wilayahRaw.provinsi : null;
  const kabupatenRaw = isObject(wilayahRaw.kabupaten) ? wilayahRaw.kabupaten : null;
  const kecamatanRaw = isObject(wilayahRaw.kecamatan) ? wilayahRaw.kecamatan : null;
...
export function mapIndustryRecordToRow(record: DbIndustryRecord): IndustryRow | null {
  ...
  if (record.platform === "Google Maps") {
    return {
      ...
      metadata: {
        ...
        reviewCount: Math.trunc(toNumber(String(metadata.reviewCount ?? 0))),
      },
    };
  }
  ...
  if (record.platform === "YouTube") {
    return {
      ...
      metadata: {
        ...
        viewCount: Math.trunc(toNumber(String(metadata.viewCount ?? 0))),
        likeCount: Math.trunc(toNumber(String(metadata.likeCount ?? 0))),
        commentCount: Math.trunc(toNumber(String(metadata.commentCount ?? 0))),
        subscriberCount: Math.trunc(toNumber(String(metadata.subscriberCount ?? 0))),
      },
    };
  }
  ...
  if (record.platform === "TikTok") {
    return {
      ...
      metadata: {
        ...
        likeCount: Math.trunc(toNumber(String(metadata.likeCount ?? 0))),
        commentCount: Math.trunc(toNumber(String(metadata.commentCount ?? 0))),
        shareCount: Math.trunc(toNumber(String(metadata.shareCount ?? 0))),
        followerCount: Math.trunc(toNumber(String(metadata.followerCount ?? 0))),
      },
    };
  }
