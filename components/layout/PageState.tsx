...
export default function PageState({
	...
	return (
		<div
			className={`border-base-300 bg-base-100 rounded-xl border px-5 py-10 text-center shadow-sm ${className}`.trim()}
		>
			{isLoading ? (
				<span className="loading loading-bars loading-lg text-primary mx-auto mb-3" />
			) : (
				<Icon className={`mx-auto mb-3 size-10 ${config.iconClass}`} />
			)}
			<h2 className="text-lg font-semibold">{title ?? config.title}</h2>
			<p className="text-base-content/70 mx-auto mt-1 max-w-md text-sm">
