...
export default function PetaIndustriMapCanvas({
	...
	const selectedMapsPoint = useMemo<GoogleMapsIndustryRow | null>(() => {
		if (!selectedPoint || selectedPoint.platform !== "Google Maps")
			return null;
	...
	const onHover = (event: MapLayerMouseEvent) => {
		...
		if (fid !== undefined && fid !== null) {
			...
			const boundaryName =
				boundaryMode === "desa"
					? feature?.properties?.kel_desa
					: feature?.properties?.kecamatan;
			onHoverBoundaryName(
				typeof boundaryName === "string" ? boundaryName : null,
			);
			return;
		}
	...
	const onMapClick = (event: MapMouseEvent) => {
		...
		if (feature.layer.id === "industry-clusters") {
			...
			const source = map.getSource("industry-points") as
				| GeoJSONSource
				| undefined;
			void source
	...
	return (
		...
		>
			<Source id="boundary-source" type="geojson" data={boundaryData}>
				...
				{layers.boundaryHover ? (
					<Layer {...boundaryHoverLayer} />
				) : null}
			</Source>
			...
			{selectedMapsPoint ? (
				...
				>
					<div
						...
						data-tip="Lokasi terpilih"
					>
						<MapPin className="text-error h-8 w-8 drop-shadow-md" />
					</div>
				</Marker>
			) : null}
